...
void TutorialTab::OnButton(wxCommandEvent &evt)
{
  ...
  switch (button_num)
  {
    ...
    case button_Check_Collision:
     ...
     /** Run */
     /*
      * All the fun happens when the run button is pressed. This transfers
      * control of the arm to a joystick connected to the computer via usb, with
      * the device accessible via /dev/input/js0.
      * When 'running' the x,y,z position of the end effector can be controlled
      * with 3 of the axis of the controller. Pressing 'Enter' on the command
      * line pauses the joystick control, and returns the travelled path back
      * to the GUI, so it can be replayed and saved.
      */
    case button_Run:
      ...
      Eigen::VectorXd start = mStartConfig, tmp;
      Eigen::VectorXd startXYZ = jt->GetXYZ(start);
      wsPath.push_back(start);
...
//        jt->getXYZ;
      		...
      		tmp = jt->GetPseudoInvJac(start)*(mTargetXYZ-startXYZ)+start;
 		      if(mWorld->getRobot(mRobotId)->setDofs( tmp, mLinks ))
          {
            mWorld->getRobot(mRobotId)->update();
            mPlanner = new PathPlanner( *mWorld, false, 0.02 );
            mLinks = mWorld->getRobot(mRobotId)->getQuickDofsIndices();
            if(mWorld->checkCollision() == false && !(tmp[1]>2.0/3.0*3.14 || tmp[1]<-2.0/3.0*3.14 || tmp[3]>2.0/3.0*3.14 || tmp[3]<-2.0/3.0*3.14 || tmp[5]>2.0/3.0*3.14 || tmp[5]<-2.0/3.0*3.14))
            {
              if((tmp-start).norm()>0.02)
              {
                if(mPlanner->planPath( mRobotId, mLinks, start, tmp, false, true, true, false, 5000))
                {
                  start = tmp;
                  std::list<Eigen::VectorXd>::iterator it=mPlanner->path.begin();
                  std::cout<<"RRT path planned. Displaying the RRT path of length"<<mPlanner->path.size()<<std::endl;
                  while(++it!=mPlanner->path.end() && ++it!=mPlanner->path.end() && ++it!=mPlanner->path.end() && ++it!=mPlanner->path.end())
                  {
                    mWorld->getRobot(mRobotId)->setDofs( *it, mLinks );
                    mWorld->getRobot(mRobotId)->update();
                    viewer->UpdateCamera();
                  }
                  wsPath.insert(wsPath.end(),mPlanner->path.begin(),mPlanner->path.end());
                  std::cout<<"The awesome list length"<<mPlanner->path.size()<<std::endl;
                  mWorld->getRobot(mRobotId)->setDofs( tmp, mLinks );
                  mWorld->getRobot(mRobotId)->update();
                  viewer->UpdateCamera();
                  startXYZ = mTargetXYZ;
                }
                ...
                {
                  mWorld->getRobot(mRobotId)->setDofs( start, mLinks );
                  mWorld->getRobot(mRobotId)->update();
                  mTargetXYZ = startXYZ;
                }
              }
